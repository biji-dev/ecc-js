...
function runTests() {
  ...
  if (test('end-to-end: --locale zh dry-run includes docs-zh-cn operations', () => {
  ...
  if (test('end-to-end: legacy language plus --locale keeps legacy install and docs', () => {
    ...
    try {
      ...
      assert.ok(json.plan.includedComponentIds.includes('locale:zh-cn'));
      assert.ok(json.plan.selectedModuleIds.includes('framework-language'));
      assert.ok(json.plan.selectedModuleIds.includes('docs-zh-cn'));
    } finally {
      fs.rmSync(homeDir, { recursive: true, force: true });
      fs.rmSync(projectDir, { recursive: true, force: true });
    }
  ...
  if (test('end-to-end: --locale zh-CN installs translated docs side-by-side', () => {
    ...
    try {
      ...
      assert.ok(
        fs.existsSync(path.join(claudeRoot, 'docs', 'zh-CN', 'README.md')),
        'Should install Simplified Chinese README under docs/zh-CN'
      );
      assert.ok(
      ...
      assert.deepStrictEqual(state.request.includeComponents, ['locale:zh-cn']);
      assert.deepStrictEqual(state.resolution.selectedModules, ['docs-zh-cn']);
    } finally {
      fs.rmSync(homeDir, { recursive: true, force: true });
      fs.rmSync(projectDir, { recursive: true, force: true });
    }
